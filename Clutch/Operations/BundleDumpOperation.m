...
- (void)main {
    @try {
        ...
        for (uint32_t i = 0; i < numHeaders; i++) {
            ...
            KJDebug(@"Finding compatible dumper for binary %@ with arch cputype: %u", originalBinary, macho.header.cputype);
            for (Class dumperClass in dumpers) {
                ...
                if ([_dumper compatibilityMode] == ArchCompatibilityNotCompatible) {
                    KJDebug(@"%@ cannot dump binary %@ (arch %@). Dumper not compatible, finding another dumper", _dumper, originalBinary, [Dumper readableArchFromHeader:macho]);
                    _dumper = nil;
                } else {
                    break;
                }
            ...
            if (_dumper == nil) {
                KJDebug(@"Couldn't find compatible dumper for binary %@ with arch %@. Will have to \"strip\".", originalBinary, [Dumper readableArchFromHeader:macho]);
                [_headersToStrip addObject:@(macho.offset)];
                continue;
            }
            KJDebug(@"Found compatible dumper %@ for binary %@ with arch %@", _dumper, originalBinary, [Dumper readableArchFromHeader:macho]);
