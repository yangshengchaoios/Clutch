...
- (BOOL)dumpBinary {
    ...
    NSString *binaryDumpPath = [self.originalBinary.workingPath stringByAppendingPathComponent:self.originalBinary.binaryPath.lastPathComponent];
    NSString *swappedBinaryPath = self.originalBinary.binaryPath;
    NSString *newSinf = self.originalBinary.sinfPath;
    NSString *newSupp = self.originalBinary.suppPath; // default values if we dont need to swap archs

    // check if cpusubtype matches
    if ((self.thinHeader.header.cpusubtype != [Device cpu_subtype]) &&
        (self.originalBinary.hasMultipleARMSlices || (self.originalBinary.hasARM64Slice && ([Device cpu_type] == CPU_TYPE_ARM64)))) {
        ...
        newSinf = [self.originalBinary.sinfPath.stringByDeletingPathExtension stringByAppendingString:[suffix stringByAppendingPathExtension:self.originalBinary.sinfPath.pathExtension]];
        newSupp = [self.originalBinary.suppPath.stringByDeletingPathExtension stringByAppendingString:[suffix stringByAppendingPathExtension:self.originalBinary.suppPath.pathExtension]];
        [self swapArch];
    }
    ...
    for (unsigned int i = 0; i < self.thinHeader.header.ncmds; i++) {
        ...
        switch (cmd) {
            ...
            case LC_ENCRYPTION_INFO: {
                ...
                foundCrypt = YES;
                KJDebug(@"FOUND ENCRYPTION INFO: cryptoff %u | cryptsize %u | cryptid %u", crypt.cryptoff, crypt.cryptsize, crypt.cryptid);
                break;
            }
            case LC_SEGMENT: {
    ...
    if (!foundCrypt || !foundSignature || !foundStartText) {
        KJDebug(@"dumping binary: some load commands were not found %@ %@ %@", foundCrypt ? @"YES" : @"NO", foundSignature ? @"YES" : @"NO", foundStartText ? @"YES" : @"NO");
        return NO;
    }
    ...
    for (uint32_t index = 0; index < countBlobs; index++) { // is this the code directory?
        if (CFSwapInt32(codesignblob->index[index].type) == CSSLOT_CODEDIRECTORY) {
            ...
            begin = self.thinHeader.offset + ldid.dataoff + CFSwapInt32(codesignblob->index[index].offset); // store the top of the codesign directory blob
    ...
    [[NSFileManager defaultManager] createSymbolicLinkAtPath:[workingPath stringByAppendingPathComponent:@"Frameworks"] withDestinationPath:self.originalBinary.frameworksPath error:nil];
